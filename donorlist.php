...
{
    echo "<table class='table table-hover table-striped table-responsive table-bordered''>
        <thead class='thead'>
        <tr>
        <td><b>Name</b></td>
        <td><b>Mobile</b></td>
    ...
    {  
        ...
        $donationdate = strtotime($row['time']);
        $now = time();
        $diff =   $now- $donationdate ;
        $days = floor($diff/(60*60*24));
        echo "<td>";
        if($days<=90){
          echo "<button class='btn btn-warning'>Not Active</button>";
        }
        else{
